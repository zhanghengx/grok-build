...
pub(super) fn auth_hint_rows(hint: &MinimalAuthHint, width: u16) -> u16 {
    match hint {
        // Header + config path + model table + base URL + API key + backend +
        // status + hint.
        MinimalAuthHint::ConfigurationRequired { .. } => 8,
...
/// Render the non-text API backend selector in the same compact style as the
/// two editable configuration fields.
fn render_config_backend(
    buf: &mut Buffer,
    area: Rect,
    y: u16,
    bottom: u16,
    ...
    let marker = if focused { "> " } else { "  " };
    let prefix = format!("{marker}api_backend: ");
    ...
    let line = Line::from(vec![
        ...
        Span::styled(format!("< {} >", api_backend_label(backend)), value_style),
    ]);
    if y < bottom {
        buf.set_line(area.x, y, &line, area.width);
    }
...
pub(super) fn render_auth(
    ...
    match hint {
        MinimalAuthHint::ConfigurationRequired {
            ...
            y = next_y;
            y = render_config_backend(
                buf,
                area,
                y,
                bottom,
                backend.clone(),
                *field == ApiConfigurationField::ApiBackend
                    && *status == ApiConfigurationStatus::Editing,
                theme,
            );
            ...
            let _ = put_line(
                ...
                Line::from(vec![
                    ...
                    Span::styled(" fields  ", gray),
                    Span::styled("left/right", bold),
                    Span::styled(" backend  ", gray),
...
mod tests {
    ...
    fn render_auth_shows_api_configuration() {
        ...
        assert!(text.contains("tab fields  left/right backend"), "{text:?}");
        assert!(
            text.contains("> api_backend: < Responses >"),
    ...
    #[test]
    fn auth_hint_preserves_backend_selection() {
        let hint = minimal_auth_hint_with_config(
        ...
        match hint {
            MinimalAuthHint::ConfigurationRequired { backend, field, .. } => {
                assert_eq!(backend, ApiBackend::Messages);
                assert_eq!(field, ApiConfigurationField::ApiBackend);
            }
            _ => panic!("expected configuration hint"),
        }
    }
