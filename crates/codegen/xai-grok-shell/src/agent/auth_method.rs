...
pub const AUTH_ERROR_API_KEY: &str =
    "API key authentication failed. Set a valid api_key or env_key in ~/.grok/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str = "preferred_method=oidc but no session is available. Set api_key or env_key in ~/.grok/config.toml instead.";
...
pub(crate) fn xai_api_key_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some(format!("api_key/env_key in config.toml"))),
    )
}
