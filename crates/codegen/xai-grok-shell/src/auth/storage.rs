...
pub fn read_token_by_scope(grok_home: &Path, scope: &str) -> anyhow::Result<String> {
    ...
    let store =
        read_auth_json(&path).map_err(|_| anyhow::anyhow!("No API key found. Set api_key or env_key in ~/.grok/config.toml."))?;
    lookup_auth(&store, scope).map(|a| a.key).ok_or_else(|| {
        anyhow::anyhow!("Your API key is invalid. Set a valid api_key or env_key in ~/.grok/config.toml.")
    })
}
