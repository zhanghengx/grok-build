#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Legacy WebLogin auth keeps its verbose message (with api_key/env_key configuration
    /// guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          configure a valid api_key or env_key in config.toml"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
